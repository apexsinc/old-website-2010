	...
	borderColor,borderWidth,borderStyle,Alpha_opacity,specType,specSpeed,lw,lh,rw,rh)
{
	...
	if(tempObj.borderStyle=="ridge")
		tempObj.borderStyle="outset";
	if(tempObj.borderStyle=="groove")
		tempObj.borderStyle="inset";
...
function endSTM()
{
	...
	for(menuIndex=0;menuIndex<STMenu[menuGroupIndex].menus.length;menuIndex++)
	{
		...
		if(menuIndex==0)
		{
			...
			if(tempObj.getMenuGroup().position=="absolute")
				innerText+=(" visibility:hidden;");
			else
				innerText+=(" visibility:visible;");
		}
		else
		{
		...
		for(itemIndex=0;itemIndex<STMenu[menuGroupIndex].menus[menuIndex].items.length;itemIndex++)
		{
			...
			case "normal":
				...
				innerItemText+=("\"");
				innerItemText+=(" BORDER="+tempObj.borderWidth);
				innerItemText+=" STYLE=\"";
				...
				if(tempObj.borderStyle=="outset"||tempObj.borderStyle=="ridge")
				{
					innerItemText+=(" border-color:"+tempObj.borderColorLightOut);
					innerItemText+=(" "+tempObj.borderColorDarkOut);
					innerItemText+=(" "+tempObj.borderColorDarkOut);
					innerItemText+=(" "+tempObj.borderColorLightOut+";");
				}
				else if(tempObj.borderStyle=="inset"||tempObj.borderStyle=="groove")
				{
					innerItemText+=(" border-color:"+tempObj.borderColorDarkOut);
					innerItemText+=(" "+tempObj.borderColorLightOut);
					innerItemText+=(" "+tempObj.borderColorLightOut);
					innerItemText+=(" "+tempObj.borderColorDarkOut+";");
				}
				...
				if(tempObj.decLtWidth!=0||tempObj.parentMenu.arrangeType=="vertically")
					innerItemText+=("<TD NOWRAP ALIGN=CENTER VALIGN=MIDDLE WIDTH="+tempObj.parentMenu.lw+" ID="+(tempObj.block+"LT")+" STYLE=\"border-width:0\">"+tempObj.getLeftDecString()+"</TD>");
				innerItemText+=("<TD NOWRAP WIDTH=100% ID="+(tempObj.block+"MT")+" STYLE=\"border-width:0\"");
				...
				if(tempObj.parentMenu.arrowWidth!=0||tempObj.parentMenu.arrangeType=="vertically")
					innerItemText+=("<TD NOWRAP ALIGN=CENTER VALIGN=MIDDLE WIDTH="+tempObj.parentMenu.rw+" ID="+(tempObj.block+"RT")+" STYLE=\"border-width:0\">"+tempObj.getArrowString()+"</TD>");
			...
			case "sepline":
				...
				else
				{
					...
					innerItemText+="<TABLE WIDTH=100% HEIGHT=100% CELLPADDING=0 CELLSPACING=0";
					innerItemText+=" ID="+tempObj.block;
					innerItemText+=" STYLE=\"";
					innerItemText+=(" background-color:"+tempObj.background_colorOut+";");
...
function showFloatMenu(name,e)
{
	...
	{
		e.cancelBubble=true;
		showFloatMenuAt(name,e.clientX+document.body.scrollLeft,e.clientY+document.body.scrollLeft);
	}
}
